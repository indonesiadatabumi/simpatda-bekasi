...
class Spreadsheet_Excel_Writer_Parser extends PEAR
{
    ...
    protected function _match($token)
    {
        switch ($token) {
            ...
            default:
                ...
                elseif (
                    preg_match("/^\w+(\:\w+)?\![A-Ia-i]?[A-Za-z][0-9]+$/u", $token) and
                    !preg_match("/[0-9]/", $this->_lookahead) and
                ...
                elseif (
                    preg_match("/^\w+(\:\w+)?\!([A-Ia-i]?[A-Za-z])?[0-9]+:([A-Ia-i]?[A-Za-z])?[0-9]+$/u", $token) and
                    !preg_match("/[0-9]/", $this->_lookahead)
                ) {
                    return $token;
                }
    ...
    protected function _func()
    {
        ...
        while ($this->_current_token != ')') {
            ...
            if ($num_args > 0) {
                if (
                    $this->_current_token == SPREADSHEET_EXCEL_WRITER_COMA or
