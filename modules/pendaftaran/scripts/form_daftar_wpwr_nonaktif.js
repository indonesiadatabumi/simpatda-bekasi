...
var completePage = function() {
	...
	$("input[name=btn_cetak]").click(function() {
		url = GLOBAL_WP_NONAKTIF_VARS["cetak"] +
				"?fDate=" + $('#fDate').val() + 
				"&tDate=" + $('#tDate').val() +
				"&bidus=" + $('#bidus').val() +
	...
	$("input[name=btn_cetak_excel]").click(function() {
		url = GLOBAL_MAIN_VARS["BASE_URL"] + "pendaftaran/dokumentasi_pengolahan/export_excel_wp_nonaktif/" +
				"?fDate=" + $('#fDate').val() + 
				"&tDate=" + $('#tDate').val() +
				"&bidus=" + $('#bidus').val() +
				"&camat=" + $('#wp_wr_kd_camat').val() +
		...
		$.download(url, 'filename=daftar_wp_tutup&format=xls');
	});
};
