...
class Dokumentasi_pengolahan extends Master_Controller {
	...
	function daftar_wpwr_nonaktif() {
		...
		$data['bidang_usaha'] = $bidang_usaha;
		$this->load->view('form_daftar_wpwr_nonaktif', $data);
	}
	...
	/**
	 * export data wp_nonaktif
	 */
	function export_wp_nonaktif() {
		$mengetahui = $this->input->get('mengetahui');
		$pemeriksa = $this->input->get('pemeriksa');
		...
		//get from database
		$query = $this->dokumentasi_pengolahan_model->get_list_wp_nonaktif();
	    $data['arr_data'] = $query;
	    ...
	    //load view pdf
	   	$this->load->view('pdf_daftar_wpwr_nonaktif', $data);
	}

	function export_excel_wp_tutup() {
		error_reporting(E_ERROR | E_WARNING | E_PARSE);
		...
		// HTTP headers
		HeaderingExcel('daftar_wp_tutup.xls');
		...
		// Creating the first worksheet
		$worksheet1 =& $workbook->add_worksheet('Daftar WP Tutup');
		$worksheet1->set_column(0,0,5);
		$worksheet1->set_column(0,1,30);
		$worksheet1->set_column(0,2,70);
		$worksheet1->set_column(0,3,20);
		...
		$worksheet1->write_string(0, 0, "Daftar Wajib Pajak Tutup");
		$worksheet1->write_string(1, 0, $fDate." - ".$tDate);
		...
		$worksheet1->write_string(4, 3, "N P W P D");
		$worksheet1->write_string(4, 4, "Pemilik");
		$worksheet1->write_string(4, 5, "Tgl. Tutup");
		...
		if ($wpwr_kdusaha != "")
		{
			$where .= " AND wp_wr_bidang_usaha='".$wpwr_kdusaha."'";
		}
		$wpwr_kdcamat = $_GET['camat'];
		if ($wpwr_kdcamat != "")
		{
		...
		if(!empty($result)) {
			...
			foreach($result as $k => $v1) {
				$alamat_lengkap = str_replace("\n","",$v1['wp_wr_almt']) . ' Kel. ' . $v1['wp_wr_lurah'] . ' Kec. ' . $v1['wp_wr_camat'];
				$worksheet1->write_string($no_baris,0,$no_urut);
				$worksheet1->write_string($no_baris,1,$v1['wp_wr_nama']);
				$worksheet1->write_string($no_baris,2,strtoupper($alamat_lengkap));
				$worksheet1->write_string($no_baris,3,$v1['npwprd']);
			  	$worksheet1->write_string($no_baris,4,$v1['wp_wr_nama_milik']);
				$worksheet1->write_string($no_baris,5,format_tgl($v1['wp_wr_tgl_tutup']));
	...
	function export_excel_wp_nonaktif() {
		error_reporting(E_ERROR | E_WARNING | E_PARSE);
		...
		// HTTP headers
		HeaderingExcel('daftar_wp_nonaktif.xls');
		...
		// Creating the first worksheet
		$worksheet1 =& $workbook->add_worksheet('Daftar WP Non Aktif');
		$worksheet1->set_column(0,0,5);
		$worksheet1->set_column(0,1,30);
		$worksheet1->set_column(0,2,70);
		$worksheet1->set_column(0,3,20);
		...
		$worksheet1->write_string(0, 0, "Daftar Wajib Pajak Non Aktif");
		$worksheet1->write_string(1, 0, $fDate." - ".$tDate);
		...
		$worksheet1->write_string(4, 3, "N P W P D");
		$worksheet1->write_string(4, 4, "Pemilik");
		$worksheet1->write_string(4, 5, "Tgl. Non Aktif");
		...
		if ($wpwr_kdusaha != "")
		{
			$where .= " AND b.wp_wr_bidang_usaha='".$wpwr_kdusaha."'";
		}
		$wpwr_kdcamat = $_GET['camat'];
		if ($wpwr_kdcamat != "")
		{
		...
		if(!empty($result)) {
			...
			foreach($result as $k => $v1) {
				...
				$worksheet1->write_string($no_baris,3,$v1['npwprd']);
			  	$worksheet1->write_string($no_baris,4,$v1['wp_wr_nama_milik']);
				$worksheet1->write_string($no_baris,5,format_tgl($v1['tgl_nonaktif']));
		...
		//insert history log ($module, $action, $description)
		$this->common_model->history_log("pendaftaran", "P", "Print Daftar Penonaktifan Wajib Pajak");
