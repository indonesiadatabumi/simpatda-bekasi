...
if(empty($linespace)) $linespace = 6;
...
$pdf->MultiCell(190,3,$pdf->SetFont('Arial','B',6).$pdf->Cell(12,3,"",'','','C').
...
$pdf->MultiCell(190,3,$pdf->SetFont('Arial','',5).$pdf->Cell(12,3,"",'','','C').
					$pdf->Cell(50,3,ucwords(strtolower($pemda->dapemda_lokasi))." - ".
					ucwords(strtolower($pemda->dapemda_ibu_kota)),'','','L').$pdf->Cell(80,3,"",'','','C').
					$pdf->Cell(48,3,"",'','','C'),'','','',0);
					
$pdf->MultiCell(190,4,$pdf->SetFont('Arial','',5).$pdf->Cell(12,4,"",'','','C').$pdf->Cell(50,3,"Telp. " .
					$pemda->dapemda_no_telp,'','','L').$pdf->SetFont('Arial','B',8).
					//$pdf->Cell(80,4,"DAFTAR INDUK ".$paret." ".strtoupper($bidus).$judulplus,'','','C').$pdf->Cell(48,4,"",'','','C'),'','','',0);
					$pdf->Cell(80,4,"WAJIB PAJAK YANG DAFTAR ",'','','C').$pdf->Cell(48,4,"",'','','C'),'','','',0);
...
$pdf->MultiCell(190,4,$pdf->SetFont('Arial','',5).$pdf->Cell(12,4,"",'','','C').$pdf->Cell(50,3,"",'','','L').
					$pdf->SetFont('Arial','B',8).$pdf->Cell(80,4,"$fDate s/d $tDate ",'','','C').
					$pdf->Cell(48,4,"",'','','C'),'','','',0);
...
$wpwr_tgldaftar = $_GET['wp_wr_tgl_kartu'];
	...
	case 2:
		...
		if(!empty($ar_data)) {
			...
			foreach($ar_data as $k => $v) {
				...
				$data1[0]['T_SIZE'] = 7;$data1[1]['T_SIZE'] = 7;
				...
				if(!empty($ardat)) {
					...
					foreach($ardat as $k1 => $v1) {
						...
						$data2[5]['TEXT'] = "";
						$data2[4]['TEXT'] = format_tgl($v1[wp_wr_tgl_kartu]);
						
						$data2[0]['T_ALIGN'] = 'R';$data2[3]['T_ALIGN'] = 'C';$data2[4]['T_ALIGN'] = 'C';$data2[5]['T_ALIGN'] = 'C';
						$data2[0]['BRD_TYPE'] = 'LT';$data2[1]['BRD_TYPE'] = 'LT';$data2[2]['BRD_TYPE'] = 'LT';$data2[3]['BRD_TYPE'] = 'LT';$data2[4]['BRD_TYPE'] = 'LT';$data2[5]['BRD_TYPE'] = 'LRT';
						$data2[0]['T_SIZE'] = 7;$data2[1]['T_SIZE'] = 7;$data2[2]['T_SIZE'] = 7;$data2[3]['T_SIZE'] = 7;$data2[4]['T_SIZE'] = 7;$data2[5]['T_SIZE'] = 7;
	...
	case 4:
		...
		if(!empty($ar_data)) {
			...
			foreach($ar_data as $k => $v) {
				...
				$data1[4]['TEXT'] = "" . format_tgl($v[wp_wr_tgl_kartu]);
				if(!empty($wpwr_kdusaha)) $data1[5]['TEXT'] = "";
				else $data1[5]['TEXT'] = strtoupper($kodus);

				$data1[0]['T_ALIGN'] = 'R';$data1[3]['T_ALIGN'] = 'C';$data1[4]['T_ALIGN'] = 'C';$data1[5]['T_ALIGN'] = 'C';
				$data1[0]['BRD_TYPE'] = 'LT';$data1[1]['BRD_TYPE'] = 'LT';$data1[2]['BRD_TYPE'] = 'LT';$data1[3]['BRD_TYPE'] = 'LT';$data1[4]['BRD_TYPE'] = 'LT';$data1[5]['BRD_TYPE'] = 'LRT';
				$data1[0]['T_SIZE'] = 7;$data1[1]['T_SIZE'] = 7;$data1[2]['T_SIZE'] = 7;$data1[3]['T_SIZE'] = 7;$data1[4]['T_SIZE'] = 7;$data1[5]['T_SIZE'] = 7;
				$data2[0]['LN_SIZE'] = $linespace;$data2[1]['LN_SIZE'] = $linespace;$data2[2]['LN_SIZE'] = $linespace;$data2[3]['LN_SIZE'] = $linespace;$data2[4]['LN_SIZE'] = $linespace;$data2[5]['LN_SIZE'] = $linespace;
				$pdf->tbDrawData($data1);
