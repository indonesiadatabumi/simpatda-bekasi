...
$pdf->MultiCell(190,4,$pdf->SetFont('Arial','',5).$pdf->Cell(12,4,"",'','','C').$pdf->Cell(50,3,"",'','','L').
					$pdf->SetFont('Arial','B',9).$pdf->Cell(190,4,"$fDate Sampai Dengan $tDate ",'','','C').
					$pdf->Cell(192,4,"",'','','C'),'','','',0);
...
$wpwr_jenis_golongan = $_GET['wp_wr_golongan'];
	...
	case 2:
		...
		if(!empty($ar_data)) {
			...
			foreach($ar_data as $k => $v) {
				...
				if (!empty($wpwr_jenis_golongan)) {
					$wherex .= " AND wp_wr_gol = '$wpwr_jenis_golongan'";
				}
...
if ($this->input->get('mengetahui') != "0" && $this->input->get('pemeriksa') != "0") {
	...
	$ttd1[0]['TEXT'] = $mengetahui->pejda_nama;
	$ttd1[1]['TEXT'] = $pemeriksa->pejda_nama;
	$pdf->tbDrawData($ttd1);
