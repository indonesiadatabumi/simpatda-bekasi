...
$th2[0]['TEXT'] = "<h2>( Dari".$f_date_lengkap." Sampai Dengan ".$t_date_lengkap." )</h2>";
$th2[0]['COLSPAN'] = 11;
$th2[0]['BRD_TYPE'] = 0;
$pdf->tbDrawData($th2);
...
function get_wp_sebelumnya($bidus, $f_date, $camat_id) {
	if (!isset($bidus))
		return 0;
		
	$sql = "SELECT count(wp_wr_id) 
			FROM wp_wr
			WHERE wp_wr_bidang_usaha='$bidus' AND wp_wr_status_aktif='t'";
	
	if (isset($f_date)) 
		$sql .= " AND wp_wr_tgl_kartu < '$f_date'";
...
$before_start_date = date('Y-m-d', strtotime(date("Y-m-d", strtotime($fDate)) . " -1 month"));
$before_end_date = date('Y-m-d', strtotime(date("Y-m", strtotime($tDate)) . " -1 day"));
...
if($_GET['bidus'] != "") {
	...
	foreach($sqlx as $k => $v) {
		...
		$jumlah_wp = $this->adodb->GetOne(get_jumlah_wp($bidus[$i], TRUE, NULL, $fDate, NULL));
	...
	$bidus = array(1 => "1", 2 => "16", 3 => "11",  5 => "14", 6 => "18");
	...
	for ($i = 1; $i <= 6; $i++) {
		...
		//$wp_sebelumnya = $this->adodb->GetOne(get_wp_sebelumnya($bidus[$i], $fDate, null));
		/*
		$wp_kurang_sebelumnya = $this->adodb->GetOne("
			SELECT COUNT(wp_wr_id) FROM wp_wr
			WHERE wp_wr_bidang_usaha='$bidus[$i]' AND wp_wr_status_aktif = 'f' and wp_wr_tgl_tutup >= '$fDate' 
			and wp_wr_tgl_kartu <= '$fDate'
		");
		*/
		//$wp_sebelumnya = $wp_sebelumnya + $wp_kurang;
		$wp_sebelumnya = $this->adodb->GetOne("
				SELECT COUNT(DISTINCT wp_wr_id)
				FROM
				(
				SELECT wp_wr_id
				FROM wp_wr
				WHERE wp_wr_bidang_usaha='$bidus[$i]' AND wp_wr_status_aktif='t' AND wp_wr_tgl_kartu < '$fDate'
