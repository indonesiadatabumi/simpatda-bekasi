...
if(empty($linespace)) $linespace = 5.0;
...
$pdf->MultiCell(190,4,$pdf->SetFont('Arial','',6).$pdf->Cell(12,4,"",'','','C').$pdf->Cell(50,3,"Telp. " .
					...
					$pdf->Cell(190,4,"DAFTAR WP NON AKTIF ".strtoupper($bidus).$judulplus,'','','C').$pdf->Cell(48,4,"",'','','C'),'','','',0);
...
$pdf->MultiCell(190,4,$pdf->SetFont('Arial','',5).$pdf->Cell(12,4,"",'','','C').$pdf->Cell(50,3,"",'','','L').
					$pdf->SetFont('Arial','B',10).$pdf->Cell(190,4,"Dari tanggal $fDate Sampai Dengan $tDate ",'','','C').
					$pdf->Cell(48,4,"",'','','C'),'','','',0);
...
$th1[5]['TEXT'] = "<b>TANGGAL NON AKTIF</b>";
if ($this->input->get('bidus') == "") {
...
if(!empty($arr_data)) {
	...
	foreach($arr_data as $k => $v) {
		...
		$data1[5]['TEXT'] = format_tgl($v['tgl_nonaktif']);
		if ($this->input->get('bidus') == "") {
