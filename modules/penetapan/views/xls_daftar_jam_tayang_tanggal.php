...
$arrbulan = array(1=>'Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember');
 ...
 // HTTP headers
HeaderingExcel('daftar_tayang_reklame_via_tgl_reklame.xls');
...
$worksheet1 =& $workbook->add_worksheet("DAFTAR TAYANG ");
...
$worksheet1->set_column(0,8,35);
$worksheet1->set_column(0,9,35);
$worksheet1->set_column(0,10,17);
$worksheet1->set_column(0,11,17);

$worksheet1->write_string(0,0, "DAFTAR TAYANG REKLAME", $ftitle); 
$worksheet1->merge_cells(0, 0, 0, 9);

// $worksheet1->write_string(1,0, "TAHUN : " . $_GET['tahun'], $ftitle); 
// $worksheet1->merge_cells(1, 0, 1, 9);

$worksheet1->write_string(2,0, "TGL KETETAPAN : ".$_GET['tgl_penetapan1']." s/d ".$_GET['tgl_penetapan2'],$ftitle); 
...
$worksheet1->write_string($no_baris,5, "LUAS",$fheader);
$worksheet1->merge_cells($no_baris, 5, $no_baris + 1, 5);
$worksheet1->write_string($no_baris,6, "JANGKA",$fheader);
$worksheet1->merge_cells($no_baris, 6, $no_baris + 1, 6);
$worksheet1->write_string($no_baris,7, "A L A M A T", $fheader);
$worksheet1->merge_cells($no_baris, 7, $no_baris + 1, 7);
$worksheet1->write_string($no_baris,8, "LOKASI PASANG", $fheader);
$worksheet1->merge_cells($no_baris, 8, $no_baris + 1, 8);
$worksheet1->write_string($no_baris,9, "JENIS REKLAME", $fheader); 
$worksheet1->merge_cells($no_baris, 9, $no_baris + 1, 9);
$worksheet1->write_string($no_baris,10, "KETETAPAN", $fheader);
$worksheet1->merge_cells($no_baris, 10, $no_baris + 1, 10);
$worksheet1->write_string($no_baris,11, "JATUH TEMPO PASANG", $fheader);
...
if (!empty($data)) {
	foreach($data as $key => $val) {
		$jatuh_tempo = date('Y-m-d', strtotime($val['netapajrek_tgl'] . ' +'.$val['sptrek_lama_pasang'].' days'));
		...
		$worksheet1->write($no_baris,5, $val['sptrek_luas'],$fdata);
		$worksheet1->write($no_baris,6, $val['sptrek_lama_pasang'],$fdata);
		$worksheet1->write($no_baris,7, $val['wp_wr_almt'],$fdata);
		if($val['wp_wr_lurah'] == null){
			$worksheet1->write($no_baris,8, $val['sptrek_lokasi'],$fdata);
		}else{
			$worksheet1->write($no_baris,8, $val['sptrek_lokasi'] . " Kel. " . $val['wp_wr_lurah'] . " Kec. " . $val['wp_wr_camat'],$fdata);
		}
		$worksheet1->write($no_baris,9, $val['korek_nama_detail'],$fdata);
		$worksheet1->write_number($no_baris,10, $val['spt_dt_pajak'],$fcurrency);
		$worksheet1->write_number($no_baris,11, ceil((strtotime($jatuh_tempo) + $ut_to_ed_diff) / $seconds_in_a_day),$fdate);
...
$worksheet1->write_string($no_baris,4, "",$fheader);
$worksheet1->write_string($no_baris,5, "", $fheader);
$worksheet1->merge_cells($no_baris, 0, $no_baris, 9);
$worksheet1->write_string($no_baris,6, "", $fheader);
$worksheet1->write_string($no_baris,7, "",$fheader);
$worksheet1->write_string($no_baris,8, "",$fheader);
$worksheet1->write_string($no_baris,9, "",$fheader);
