...
$pdf->MultiCell(190,3,$pdf->SetFont('Arial','B',7).
						...
						$pdf->SetFont('Arial','B',10).$pdf->Cell(140,4,"DAFTAR REKLAME HABIS MASA TAYANG",'','','C').
						$pdf->Cell(48,3,"",'','','C'),'','','',0);
...
if($kelurahan != null || $kelurahan != "") 
	$pdf->Cell(310,4,$pdf->SetFont('Arial','',10)."Kelurahan     : ".strtoupper($kelurahan),0,0);
...
$header1[9]['TEXT'] = "<b>JATUH TEMPO PASANG</b>";
...
if (!empty($data)) {
	...
	foreach($data as $key => $value) {
		...
		$jatuh_tempo = date('Y-m-d', strtotime($value['netapajrek_tgl'] . ' +'.$value['sptrek_lama_pasang'].' days'));
		$tbl2[0]['TEXT'] = $counter;
		...
		$tbl2[9]['TEXT'] = format_tgl($jatuh_tempo);
		$tbl2[10]['TEXT'] = "";
