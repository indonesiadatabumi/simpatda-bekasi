...
class Stpd_model extends CI_Model {
	 ...
	 * get_kecamtan
	 */
	function get_kecamatan($jenis_laporan, $jenis_pajak, $kecamatan_id = NULL, $first_param, $second_param, $status_spt, $jenis_restoran , $type_cetak = 0) {			
		if ($type_cetak == 0) {
		...
		//			$where .= " AND COALESCE(sptresto_jenis, 1)='".$jenis_restoran."'";
					...
					FROM v_stpd2 a LEFT JOIN wp_wr_detail c on a.stpd_wp_id=c.wp_wr_id
	...
	function get_data_rekapitulasi($jenis_laporan, $jenis_pajak, $camat_id, $first_param, $second_param, $status_spt, $jenis_restoran ) {
		$explode = explode('-', $second_param);
		$periode = $explode[2];
		if ($jenis_laporan == 1) {
			$where = "WHERE a.stpd_jenis_pajak IS NOT NULL and b.setorpajret_spt_periode='$periode' ";
		}else {
	...
	//		if ($jenis_restoran == 1 || $jenis_restoran == 2) {
		if ($jenis_pajak == 2) {
			...
			if ($jenis_restoran == 0) {		
				...
				$sql = "SELECT a.*, b.skbh_no,b.setorpajret_no_spt
				FROM v_stpd2 a left join v_rekapitulasi_penerimaan_detail b on b.setorpajret_no_spt=a.stpd_nomor_spt and 
				LEFT JOIN wp_wr_detail c on a.stpd_wp_id=c.wp_wr_id
				$where AND (c.wp_jenis_restoran='0' or c.wp_jenis_restoran='1' or c.wp_jenis_restoran='2') ORDER BY a.spt_nomor_spt ASC
				";
			}  else if  ($jenis_restoran == 1 ) {
				$sql = "SELECT a.*, b.skbh_no,b.setorpajret_no_spt
				FROM v_stpd2 a left join v_rekapitulasi_penerimaan_detail b on b.setorpajret_no_spt=a.stpd_nomor_spt and 
				LEFT JOIN wp_wr_detail c on a.stpd_wp_id=c.wp_wr_id
				$where AND (c.wp_jenis_restoran='0' or c.wp_jenis_restoran='1') ORDER BY a.spt_nomor_spt ASC
				";
			}else if  ($jenis_restoran == 2 ) {
			...
			$sql = "SELECT a.*, b.skbh_no,b.setorpajret_no_spt
				FROM v_stpd2 a left join v_rekapitulasi_penerimaan_detail b on b.setorpajret_no_spt=a.stpd_nomor_spt and 
				LEFT JOIN wp_wr_detail c on a.stpd_wp_id=c.wp_wr_id
				$where AND  c.wp_jenis_restoran='2' ORDER BY a.spt_nomor_spt ASC
				";	
			...
			$sql = "SELECT a.*, b.skbh_no,b.setorpajret_no_spt
FROM v_stpd2 a left join v_rekapitulasi_penerimaan_detail b on b.setorpajret_no_spt=a.stpd_nomor_spt and a.stpd_wp_id=b.setorpajret_id_wp
$where ";	
		}
		// var_dump($sql);die;
		$query = $this->db->query($sql);
