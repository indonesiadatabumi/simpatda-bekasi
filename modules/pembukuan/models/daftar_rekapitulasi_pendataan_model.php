...
class Daftar_rekapitulasi_pendataan_model extends CI_Model {
	...
	function get_data_rekapitulasi($jenis_laporan, $jenis_pajak, $camat_id, $lurah_id, $first_param, $second_param, $status_spt, $jenis_restoran) {
		...
		if ($jenis_laporan == 1) {
			...
			if ($first_param != null && $second_param != null)
			$where .= " AND v_daftar_rekapitulasi.spt_tgl_proses BETWEEN '".format_tgl($first_param)."' AND '".format_tgl($second_param)."'";
		} else {
			if ($first_param != null && $second_param != null)
			$where .= " AND EXTRACT(MONTH FROM v_daftar_rekapitulasi.spt_tgl_proses) = $first_param
					AND EXTRACT(YEAR FROM v_daftar_rekapitulasi.spt_tgl_proses) = $second_param";	
		}
