...
class Daftar_rekapitulasi_model extends CI_Model {
	 ...
	 * get_kelurahan
	 */
	function get_kelurahan($jenis_laporan, $jenis_pajak, $kecamatan_id, $kelurahan_id = NULL, $first_param, $second_param, $status_spt, $jenis_restoran = 0, $type_cetak = 0) {			
		if ($type_cetak == 0) {
			$where = "WHERE spt_jenis_pajakretribusi IS NOT NULL ";
			if ($jenis_pajak != "") 
				$where .= " AND spt_jenis_pajakretribusi='$jenis_pajak'";
			...
			if (isset($kelurahan_id) && !empty($kelurahan_id)) {
				if($jenis_pajak != '4'){
					$where .= " AND wp_wr_kd_lurah='$kelurahan_id'";
				}elseif($jenis_pajak == '4'){
					$where .= " AND wp_wr_lurah='$kelurahan_id'";
				}
			} 
			...
			if ($jenis_pajak == "2") {
				if ($jenis_restoran == 1 || $jenis_restoran == 2) {
					...
					$sql = "SELECT DISTINCT wp_wr_kd_lurah lurah_id, wp_wr_lurah lurah_nama
					FROM v_daftar_rekapitulasi2 LEFT JOIN spt_restoran ON v_daftar_rekapitulasi.spt_id=spt_restoran.sptresto_id_spt
					$where
		...
		{
			if (isset($kelurahan_id) && !empty($kelurahan_id)) {
				$where .= " AND lurah_id='$kelurahan_id'";
			}
			
			$sql = "SELECT *
					FROM kelurahan
				
					ORDER BY lurah_kode ASC";
		}
	...
	function get_data_rekapitulasi($jenis_laporan, $jenis_pajak, $camat_id, $lurah_id, $first_param, $second_param, $status_spt, $jenis_restoran) {
		
		$where = "WHERE v_daftar_rekapitulasi.spt_jenis_pajakretribusi IS NOT NULL ";
		if ($jenis_pajak != null) 
			$where .= " AND v_daftar_rekapitulasi.spt_jenis_pajakretribusi='$jenis_pajak'";
		
		if ($camat_id != null)
			if ($jenis_pajak == '4') {
				$where .= " AND v_daftar_rekapitulasi.wp_wr_camat='$camat_id'";
			}else{
				$where .= " AND v_daftar_rekapitulasi.wp_wr_kd_camat='$camat_id'";
			}

		if ($lurah_id != null)
			if ($jenis_pajak == '4') {
				$where .= " AND v_daftar_rekapitulasi.wp_wr_lurah='$lurah_id'";
			}else{
				$where .= " AND v_daftar_rekapitulasi.wp_wr_kd_lurah='$lurah_id'";
			}
			
			
		if ($jenis_laporan == 1) {
			if ($first_param != null && $second_param != null)
			$where .= " AND v_daftar_rekapitulasi.skbh_tgl BETWEEN '".format_tgl($first_param)."' AND '".format_tgl($second_param)."'";
		} else {
			if ($first_param != null && $second_param != null)
			$where .= " AND EXTRACT(MONTH FROM v_daftar_rekapitulasi.spt_periode_jual1) = $first_param
					AND EXTRACT(YEAR FROM v_daftar_rekapitulasi.spt_periode_jual1) = $second_param";	
		}
			
		if ($status_spt != null)
			$where .= " AND v_daftar_rekapitulasi.ketspt_id='$status_spt'";
			
		//jika jenis pajak restoran
		if ($jenis_pajak == "2") {
			if ($jenis_restoran == 1 || $jenis_restoran == 2) {
				$where .= " AND COALESCE(spt_restoran.sptresto_jenis, 1)='".$jenis_restoran."'";
				
				$sql = "SELECT *
				FROM v_daftar_rekapitulasi LEFT JOIN spt_restoran ON v_daftar_rekapitulasi.spt_id=spt_restoran.sptresto_id_spt
				LEFT JOIN setoran_pajak_retribusi ON v_daftar_rekapitulasi.spt_id = setoran_pajak_retribusi.setorpajret_id_spt AND v_daftar_rekapitulasi.spt_periode = setoran_pajak_retribusi.setorpajret_spt_periode
				LEFT JOIN spt_detail ON v_daftar_rekapitulasi.spt_id=spt_detail.spt_dt_id_spt
				LEFT JOIN kode_rekening_new ON spt_detail.spt_dt_korek=kode_rekening_new.korek_id
				$where
				ORDER BY v_daftar_rekapitulasi.spt_nomor ASC";
			} else {
				$sql = "SELECT *
			...
			$sql = "SELECT 
						v_daftar_rekapitulasi.spt_id,
						v_daftar_rekapitulasi.spt_periode,
						v_daftar_rekapitulasi.wp_wr_nama, 
						v_daftar_rekapitulasi.npwprd, 
						v_daftar_rekapitulasi.wp_wr_almt, 
						v_daftar_rekapitulasi.spt_periode_jual1, 
						v_daftar_rekapitulasi.spt_tgl_proses, 
						v_daftar_rekapitulasi.spt_nomor,
						v_daftar_rekapitulasi.spt_pajak,
						v_daftar_rekapitulasi.skbh_no,
						v_daftar_rekapitulasi.skbh_tgl,
						v_daftar_ketetapan_list.netapajrek_tgl_jatuh_tempo, 
						spt_reklame.sptrek_judul, 
						spt_reklame.sptrek_lokasi, 
						spt_reklame.sptrek_luas, 
						spt_reklame.sptrek_lama_pasang,
						setoran_pajak_retribusi.setorpajret_jlh_bayar,
						v_spt_detail.korek_nama
					FROM v_daftar_rekapitulasi
					LEFT JOIN v_daftar_ketetapan_list on v_daftar_rekapitulasi.spt_id=v_daftar_ketetapan_list.netapajrek_id
					LEFT JOIN v_spt_detail ON v_daftar_ketetapan_list.netapajrek_id_spt=v_spt_detail.spt_dt_id_spt
					LEFT JOIN spt_reklame on v_spt_detail.spt_dt_id=spt_reklame.sptrek_id_spt_dt
					LEFT JOIN setoran_pajak_retribusi ON v_daftar_rekapitulasi.spt_id = setoran_pajak_retribusi.setorpajret_id_spt AND v_daftar_rekapitulasi.spt_periode = setoran_pajak_retribusi.setorpajret_spt_periode
					$where 
					ORDER BY v_daftar_rekapitulasi.spt_nomor ASC";
	...
	function get_data_rekapitulasi_realisasi($jenis_laporan, $jenis_pajak, $camat_id, $lurah_id, $first_param, $second_param, $status_spt, $jenis_restoran) {
	...
	function cek_bayar($spt_id, $spt_kode_billing, $spt_periode)  {
		$sql_bayar = "SELECT 
						skbh_no,
						skbh_tgl,
