 ...
 * @package Simpatda
 */
class Realisasi_tunggakan_model extends CI_Model {
	/**
	 * keterangan spt
	 ...
	 * get_kecamtan
	 */
	function get_kecamatan($jenis_pajak, $kecamatan_id = NULL, $bulan_pajak, $tahun_pajak, $status_spt, $jenis_restoran = 0, $type_cetak = 0) {			
		if ($type_cetak == 0) {
			...
			if ($bulan_pajak != "" && $tahun_pajak != "")
				$where .= " AND EXTRACT(MONTH FROM spt_periode_jual1) = $bulan_pajak
						AND EXTRACT(YEAR FROM spt_periode_jual1) = $tahun_pajak";
	 ...
	 * get data spt
	 */
	function get_data_rekapitulasi($jenis_pajak, $camat_id, $bulan_pajak, $tahun_pajak, $status_spt, $jenis_restoran = 0, $jenis_daftar = 0) {	
		$where = "WHERE spt_jenis_pajakretribusi IS NOT NULL ";
		if ($jenis_pajak != null) 
			$where .= " AND spt_jenis_pajakretribusi='$jenis_pajak'";
		...
		if ($bulan_pajak != null && $tahun_pajak != null)
			$where .= " AND EXTRACT(MONTH FROM spt_periode_jual1) = $bulan_pajak
					AND EXTRACT(YEAR FROM spt_periode_jual1) = $tahun_pajak";
		...
		if ($jenis_daftar === "1" || $jenis_daftar === "2") {
			if ($jenis_daftar === "1") 
				$where .= " AND setoran IS NOT NULL";
			else 
				$where .= " AND setoran IS NULL";
		}
