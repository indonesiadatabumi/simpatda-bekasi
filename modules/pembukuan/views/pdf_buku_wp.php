...
if (!empty($dt_rows)) {
	...
	$pdf->Image("assets/images/logo-kotabekasi.JPG",13,14,14);
    ...
    $pdf->Cell(82,5,"P E N Y E T O R A N",1,0,'C');
    $pdf->Cell(28,5,"",1,2,'C');
       
    $pdf->Ln(0);
    $pdf->Cell(7,5,"No.",1,0,'C');
    $pdf->Cell(28,5,"REKENING",1,0,'C');
    $pdf->Cell(35,5,"U R A I A N",1,0,'C');
    ...
    $pdf->Cell(7,1,"",'LR',0,'C').$pdf->Cell(28,1,"",'R',0,'C').$pdf->Cell(35,1,"",'R',0,'C').$pdf->Cell(50,1,"",'R',0,'C').$pdf->Cell(17,1,"",'R',0,'C').$pdf->Cell(17,1,"",'R',0,'C').$pdf->Cell(28,1,"",'R',0,'C').$pdf->Cell(28,1,"",'R',0,'C').$pdf->Cell(17,1,"",'R',0,'C').$pdf->Cell(17,1,"",'R',0,'C').$pdf->Cell(20,1,"",'R',0,'C').$pdf->Cell(28,1,"",'R',0,'C').$pdf->Cell(28,1,"",'R',2,'C');
    $pdf->Ln(0);
    $total_tetap = 0;
    $total_setor = 0;
    $total_denda = 0;
	...
	foreach ($dt_rows as $vw) {  
        ...
        if ($vw['setorpajret_jlh_bayar'] != null && $vw['setorpajret_jlh_bayar'] > $vw['spt_pajak']) {
            $denda = $vw['setorpajret_jlh_bayar'] - $vw['spt_pajak'];
        }
	    ...
	    if($_GET['jenis_pemungutan'] == 2) {
        	...
        	$pdf->MultiCell(310, 5, $pdf->Cell(7,5,$counter,'LR',0,'L').
        							...
        							$pdf->Cell(35,5,$vw['korek_nama'],'R',0,'L').
                                    ...
                                    $pdf->Cell(20,5,format_currency($denda),'R',0,'R').
    ...
    // 	$pdf->MultiCell(310,5,$pdf->Cell(7+28+50+55,5,"T O T A L  S E L U R U H N Y A",'LTB',0,'C').$pdf->Cell(17+17,5,'','TB').$pdf->Cell(28,5,number_format ($total_tetap, 2,',','.'),'TB',0,'R').$pdf->Cell(28+17+17,5,"",'TB',0,'C').$pdf->Cell(28,5,number_format ($total_setor, 2,',','.'),'TB',0,'R').$pdf->Cell(28,5,number_format ($total_sisa, 2,',','.'),$b.'TRB',0,'R'),'','','L');
  	// } else {
    // 	$pdf->MultiCell(310,5,$pdf->Cell(7+28+35+50,5,"T O T A L  S E L U R U H N Y A",'LTB',0,'C').$pdf->Cell(17+17+28,5,'','TB').$pdf->Cell(28,5,number_format ($total_tetap, 2,',','.'),'TB',0,'R').$pdf->Cell(17+17,5,"",'TB',0,'C').$pdf->Cell(20,5,number_format ($total_denda, 2,',','.'),'TB',0,'R').$pdf->Cell(28,5,number_format ($total_setor, 2,',','.'),'TB',0,'R').$pdf->Cell(28,5,number_format ($total_sisa, 2,',','.'),$b.'TRB',0,'R'),'','','L');
	// }
    $pdf->MultiCell(310,5,$pdf->Cell(7+28+35+50,5,"T O T A L  S E L U R U H N Y A",'LTB',0,'C').$pdf->Cell(17+17+28,5,'','TB').$pdf->Cell(28,5,number_format ($total_tetap, 2,',','.'),'TB',0,'R').$pdf->Cell(17+17,5,"",'TB',0,'C').$pdf->Cell(20,5,number_format ($total_denda, 2,',','.'),'TB',0,'R').$pdf->Cell(28,5,number_format ($total_setor, 2,',','.'),'TB',0,'R').$pdf->Cell(28,5,number_format ($total_sisa, 2,',','.'),$b.'TRB',0,'R'),'','','L');
