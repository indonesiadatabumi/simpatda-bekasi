...
class Daftar_rekapitulasi extends Master_Controller {
	...
	function cetak() {
		...
		{
			if ($this->input->get('lap') == "realisasi")//jika menu realisasi
			{
				if($this->input->get('status_spt') == "11")//jika skpdkb
					$this->cetak_daftar_skpdkb(0,1);
				else if ($this->input->get('status_spt') == "3")//jika stpd
	...
	function cetak_daftar($type_cetak = 0, $lap_realisasi = 1)
	{
		...
		// HTTP headers
		if($lap_realisasi == 1){//jika menu realisasi
			HeaderingExcel('daftar-rekapitulasi-realisasi.xls');
		}else{
			HeaderingExcel('daftar-rekapitulasi-masapajak.xls');
		}
		...
		$fkecamatan->set_size(12);
		$fkecamatan->set_bold();
		$fkecamatan->set_border(1);
		$fkecamatan->set_align('left');
		$fkecamatan->set_font("Trebuchet MS");
		
		$fkelurahan = $workbook->add_format();
		$fkelurahan->set_size(10);
		$fkelurahan->set_border(1);
		$fkelurahan->set_align('left');
		$fkelurahan->set_font("Trebuchet MS");
		...
		if ($lap_realisasi == 1)
			$worksheet1->write_string(1, 0, "Berdasarkan Tanggal Realisasi " . format_tgl($this->input->get('fDate'), true, true, false) . " - " .
		...
		if ($_GET['status_spt'] == "8") {//jika sptpd
			$worksheet1->merge_cells(0, 0, 0, 15);
			$worksheet1->merge_cells(1, 0, 1, 15);
			...
			$worksheet1->merge_cells(3, 9, 3, 13);
			$worksheet1->write_string(3, 14, "TUNGGAKAN (Rp)", $fheader);
			$worksheet1->merge_cells(3, 14, 4, 14);
			$worksheet1->write_string(3, 15, "KETERANGAN", $fheader);
			$worksheet1->merge_cells(3, 15, 4, 15);
			...
			$worksheet1->write_string(4, 11, "PAJAK", $fheader);
			$worksheet1->write_string(4, 12, "DENDA", $fheader);
			$worksheet1->write_string(4, 13, "SETORAN (Rp)", $fheader);
			$worksheet1->write_blank(4, 14, $fheader);
			$worksheet1->write_blank(4, 15, $fheader);
		...
		} elseif($_GET['jenis_pajak'] == "4") {//jika pajak reklame
			$worksheet1->merge_cells(0, 0, 0, 18);
			$worksheet1->merge_cells(1, 0, 1, 18);
			...
			$worksheet1->write_blank(3, 16, $fheader);
			$worksheet1->merge_cells(3, 12, 3, 16);
			$worksheet1->write_string(3, 17, "TUNGGAKAN (Rp)", $fheader);
			$worksheet1->merge_cells(3, 17, 4, 17);
			$worksheet1->write_string(3, 18, "KETERANGAN", $fheader);
			$worksheet1->merge_cells(3, 18, 4, 18);
			...
			$worksheet1->write_string(4, 14, "PAJAK", $fheader);
			$worksheet1->write_string(4, 15, "DENDA", $fheader);
			$worksheet1->write_string(4, 16, "SETORAN (Rp)", $fheader);
			$worksheet1->write_blank(4, 17, $fheader);
			$worksheet1->write_blank(4, 18, $fheader);
			...
			$worksheet1->write_number(5, 17, "18", $fheader_small);
			$worksheet1->write_number(5, 18, "19", $fheader_small);
		}else{
			...
			$worksheet1->write_string(3, 13, "KETERANGAN", $fheader);
		...
		$no_baris = 6;
		$no_urut = 1;
		if ($_GET['jenis_pajak'] != "4") {//jika bukan pajak reklame
			//load data kecamatan
			...
			if (count($dt_kecamatan) > 0) {//jika jumlah kecamatan lebih dari 0
					...
					if ($this->input->get('lurah_id') == "") {
						...
						if ($_GET['status_spt'] == "8") {//jika sptpd
							foreach ($dt_rekap as $row) {
								...
								if ($row['skbh_tgl'] != NULL)//jika variabel skbh_tgl ada
								...
								$worksheet1->write_number($no_baris, 11, $row['spt_pajak'], $fcurrency);
	
								if($row['setorpajret_jlh_bayar'] != NULL)//jika variabel setorpajret_jlh_bayar ada
									$worksheet1->write_formula($no_baris, 12, "=N$baris-I$baris", $fsisa);
								else
									$worksheet1->write_blank($no_baris, 12, $fdata);
	
								if ($row['setorpajret_jlh_bayar'] != NULL)//jika variabel skbh_tgl ada
									$worksheet1->write_number($no_baris, 13, $row['setorpajret_jlh_bayar'], $fcurrency);
								else
									$worksheet1->write_blank($no_baris, 13, $fdata);
	
								$worksheet1->write_formula($no_baris, 14, "=(I$baris+M$baris)-N$baris", $fsisa);
								if($row['spt_dt_korek'] != NULL)//jika variabel spt_dt_korek ada
									$worksheet1->write_string($no_baris, 15, $row['korek_nama'], $fdata);
								else
							...
							foreach ($dt_rekap as $row) {
								...
								if($row['spt_dt_korek'] != NULL)//jika variabel spt_dt_korek ada
									$worksheet1->write_string($no_baris, 13, $row['korek_nama'], $fdata);
								else
									$worksheet1->write_blank($no_baris, 13, $fdata);
						...
						//load data kelurahan
						$dt_kelurahan = $this->daftar_rekapitulasi_model->get_kelurahan(
							$lap_realisasi,
							$this->input->get('jenis_pajak'),
							$camat['camat_id'],
							$lurah[0],
						...
						if (count($dt_kelurahan) > 0) {
							foreach ($dt_kelurahan as $lurah) {

								$worksheet1->write_blank($no_baris, 0, $fkelurahan);
								$worksheet1->write_string($no_baris, 1, $lurah['lurah_nama'], $fkelurahan);
								$worksheet1->write_blank($no_baris, 2, $fkelurahan);
								if ($_GET['status_spt'] == "8") {//jika sptpd
									for ($i = 3; $i <= 11; $i++)
										$worksheet1->write_blank($no_baris, $i, $fkelurahan);
								} else {
									for ($i = 3; $i <= 10; $i++)
										$worksheet1->write_blank($no_baris, $i, $fkelurahan);
								}
				...
				if ($_GET['status_spt'] == "8") {//jika sptpd
					...
					$worksheet1->write_formula($no_baris, 13, "=SUM(N7:N$no_baris)", $ftotal_currency);
					$worksheet1->write_formula($no_baris, 14, "=SUM(O7:O$no_baris)", $ftotal_sisa);
					$worksheet1->write_blank($no_baris, 15, $ftotal);
				} else {
					//bagian total
					$worksheet1->write_string($no_baris, 0, "TOTAL", $ftotal);
					$worksheet1->write_blank($no_baris, 1, $ftotal);
			...
			$tot_setoran = 0;
			$tot_tunggakan = 0;
			if($this->input->get('camat_id') != ""){//jika pilih kecamatan
				...
				if ($this->input->get('lurah_id') == ""){ //apabila lurah tidak dipilih
					//get data wp
					if ($lap_realisasi == 1){
						$dt_rekap = $this->daftar_rekapitulasi_model->get_data_rekapitulasi_realisasi(
							1,
					...
					foreach ($dt_rekap as $row) {
						...
						$worksheet1->write_string($no_baris, 2, $row['npwprd'], $fdata_center);
						$worksheet1->write_string($no_baris, 3, strtoupper($row['wp_wr_almt']), $fdata);
						$worksheet1->write_number($no_baris, 4, ceil((strtotime($row['spt_periode_jual1']) + $ut_to_ed_diff) / $seconds_in_a_day), $fdate);
						$worksheet1->write_number($no_baris, 5, ceil((strtotime($row['spt_tgl_proses']) + $ut_to_ed_diff) / $seconds_in_a_day), $fdate);
						$worksheet1->write_string($no_baris, 6, $row['spt_nomor'], $fdata_center);
						$worksheet1->write_number($no_baris, 7, $row['spt_pajak'], $fcurrency);
						...
						if ($row['skbh_no'] != NULL)//jika variabel skbh_tgl tidak null
							$worksheet1->write_string($no_baris, 12, $row['skbh_no'], $fdata_center);
						else
							$worksheet1->write_blank($no_baris, 12, $fdata_center);

						$baris = $no_baris + 1;

						if ($row['skbh_tgl'] != NULL)//jika variabel skbh_tgl tidak null
						...
						$worksheet1->write_number($no_baris, 14, $row['spt_pajak'], $fcurrency);

						if($row['setorpajret_jlh_bayar'] != NULL)//jika variabel setorpajret_jlh_bayar tidak null
								$worksheet1->write_formula($no_baris, 15, "=Q$baris-H$baris", $fsisa);
						else
							$worksheet1->write_blank($no_baris, 15, $fdata);

						if ($row['setorpajret_jlh_bayar'] != NULL)//jika variabel skbh_tgl tidak null
							$worksheet1->write_number($no_baris, 16, $row['setorpajret_jlh_bayar'], $fcurrency);
						else
							$worksheet1->write_blank($no_baris, 16, $fdata);

						$worksheet1->write_formula($no_baris, 17, "=(H$baris+P$baris)-Q$baris", $fsisa);
						if($row['korek_nama'] != NULL)//jika variabel korek_nama ada
							$worksheet1->write_string($no_baris, 18, $row['korek_nama'], $fdata);
						else
							$worksheet1->write_blank($no_baris, 18, $fdata);
					...
					$worksheet1->write_blank($no_baris, 6, $ftotal);
					$worksheet1->merge_cells($no_baris, 0, $no_baris, 6);
					$worksheet1->write_formula($no_baris, 7, "=SUM(H7:H$no_baris)", $fcurrency);
					$worksheet1->write_blank($no_baris, 8, $ftotal);
					$worksheet1->write_blank($no_baris, 9, $ftotal);
					$worksheet1->write_blank($no_baris, 10, $ftotal);
					...
					$worksheet1->write_blank($no_baris, 13, $ftotal);
					$worksheet1->merge_cells($no_baris, 8, $no_baris, 13);
					$worksheet1->write_formula($no_baris, 14, "=SUM(O7:O$no_baris)", $fcurrency);
					$worksheet1->write_formula($no_baris, 15, "=SUM(P7:P$no_baris)", $fcurrency);
					$worksheet1->write_formula($no_baris, 16, "=SUM(Q7:Q$no_baris)", $fcurrency);
					$worksheet1->write_formula($no_baris, 17, "=SUM(R7:R$no_baris)", $ftotal_sisa);
				}else {
					$lurah = explode('|', $this->input->get('lurah_id'));
					//load data kelurahan
					$dt_kelurahan = $this->daftar_rekapitulasi_model->get_kelurahan(
	...
	function cetak_pendataan($type_cetak = 0, $pendataan = 1)
	{
		...
		if ($_GET['jenis_pajak'] != "4") {
			...
			if (count($dt_kecamatan) > 0) {
				...
				foreach ($dt_kecamatan as $camat) {
					...
					if ($this->input->get('lurah_id') == "") {
						...
						$lurah = explode('|', $this->input->get('lurah_id'));
						//load data kelurahan
						$dt_kelurahan = $this->daftar_rekapitulasi_pendataan_model->get_kelurahan(
							$lap_realisasi,
							$this->input->get('jenis_pajak'),
							$camat['camat_id'],
							$lurah[0],
			...
			if($this->input->get('camat_id') != ""){//jika pilih kecamatan
				...
				//get data wp
				if ($pendataan == 1){//jika menu realisasi
					$dt_rekap = $this->daftar_rekapitulasi_pendataan_model->get_data_rekapitulasi(
						1,
						$this->input->get('jenis_pajak'),
						NULL,
					...
					if ($type_cetak == 1) {
						...
						if ($lap_realisasi == 1) {
							...
							foreach ($dt_rekap as $row) {
								...
								$worksheet1->writeString($no_baris, 4,  date('F', strtotime($row['spt_periode_jual1'])), $fdata_center);
					...
					if ($type_cetak == 1) {
						...
						if ($lap_realisasi == 1) {
							...
							foreach ($dt_rekap as $row) {
								...
								$worksheet1->writeString($no_baris, 2, $row['npwprd'], $fdata_center);
								$worksheet1->writeString($no_baris, 3, strtoupper($row['wp_wr_almt']), $fdata);
								$worksheet1->writeString($no_baris, 4,  date('F', strtotime($row['stpd_periode_jual1'])), $fdata_center);
