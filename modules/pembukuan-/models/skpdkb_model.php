 ...
 * @package Simpatda
 */
class Skpdkb_model extends CI_Model {
	/**
	 * keterangan spt
	...
	function get_kecamatan($jenis_laporan, $jenis_pajak, $kecamatan_id = NULL, $first_param, $second_param, $status_spt, $jenis_restoran , $type_cetak = 0) {			
		...
		if ($jenis_pajak == "2") {
			if ($jenis_restoran == 0 || $jenis_restoran == 1 || $jenis_restoran == 2) {
					
					$sql = "SELECT DISTINCT wp_wr_kd_camat camat_id, wp_wr_camat camat_nama
					FROM v_skpdkb
...
function get_data_rekapitulasi($jenis_laporan, $jenis_pajak, $camat_id, $first_param, $second_param, $status_spt, $jenis_restoran ) {	
	//	$thn_per=substr($second_param,6,4);
		$where = "WHERE spt_jenis_pajakretribusi IS NOT NULL ";
		if ($jenis_pajak != null) 
			$where .= " AND spt_jenis_pajakretribusi='$jenis_pajak'";
		...
		if ($jenis_pajak == "2") {
			if ($jenis_restoran == 0 || $jenis_restoran == 1 || $jenis_restoran == 2) {
			//	$where .= " AND COALESCE(b.sptresto_jenis, 1)='".$jenis_restoran."'";
				
				$sql = "SELECT * FROM v_skpdkb
