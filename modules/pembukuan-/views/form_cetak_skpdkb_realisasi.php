...
header("Content-Disposition:attachment;filename=skpdkb_realisasi.xls");
header("Expires:0");
...
<body>
<div style="font-size:12px; text-align:center; font-weight:bold; width:1242px; ">DAFTAR REKAPITULASI SKPDKB DAN REALISASI PAJAK <?= $jenis_pajak;?></div>
<div style="font-size:12px; text-align:center; font-weight:bold; width:1242px; ">Tanggal Realisasi  <?php echo format_tgl($first_param, true, true, false)." - ".format_tgl($this->input->get('tDate'), true, true, false);?></div>
...
<tbody>
	...
	if($camat_id==""){
		$qcamat="SELECT distinct wp_wr_kd_camat as camat_kode, wp_wr_camat as camat_nama FROM v_daftar_ketetapan_list 
				...
				AND netapajrek_tgl between '".format_tgl($first_param)."' AND '".format_tgl($second_param)."'  
				";
	}else {
		$qcamat="SELECT distinct wp_wr_kd_camat as camat_kode, wp_wr_camat as camat_nama FROM v_daftar_ketetapan_list 
...
//		if ($jenis_laporan == 1) {
			...
			$where .= " AND netapajrek_tgl between '".format_tgl($first_param)."' AND '".format_tgl($second_param)."' ";
/*		} else {
			if ($first_param != null && $second_param != null)
