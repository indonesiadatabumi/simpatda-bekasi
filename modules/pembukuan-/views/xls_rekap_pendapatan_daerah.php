...
$tahun_anggaran = $arr_tgl_proses[2];
 ...
 // HTTP headers
HeaderingExcel('rekap_laporan_'.date('Ym').".xls");
...
$worksheet1 =& $workbook->add_worksheet('Rekap Pendapatan Daerah');
...
$worksheet1->write_string(0, 6, "DAFTAR : ",$ftitle_right);
$worksheet1->write_string(0, 7, "REKAPITULASI PENDAPATAN DAERAH KOTA BEKASI", $ftitle);
...
$rekening = $realisasi_model->get_rekening_rekap();
if (count($rekening) > 0) {
	$target = $realisasi_model->get_target_anggaran($tahun_anggaran);
	
	$realisasi_minggu_lalu = $realisasi_model->realisasi_pajak(
								format_tgl($tgl_proses), $tahun_anggaran, 1, NULL, NULL, NULL, NULL, TRUE
							);
	//print_r($realisasi_minggu_lalu);	
	$realisasi_minggu_ini = $realisasi_model->realisasi_pajak(
								format_tgl($tgl_proses), $tahun_anggaran, 2, NULL, NULL, NULL, NULL, TRUE
							);
