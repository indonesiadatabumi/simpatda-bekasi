...
class Daftar_rekapitulasi extends Master_Controller {
		...
		if ($_GET['jenis_pajak'] != "4") {			
			...
			if (count($dt_kecamatan) > 0) {
				...
				if ($_GET['status_spt'] == "8") {
					...
					$worksheet1->merge_cells($no_baris, 8, $no_baris, 10);$worksheet1->write_blank($no_baris,10, $ftotal);
	...
	function cetak_daftar_skpdkb($type_cetak = 0, $lap_realisasi = 1) {
		...
		// HTTP headers
		HeaderingExcel('daftar-rekapitulasi-skdbkb.xls');
				...
				foreach ($dt_kecamatan as $camat) {
					...
					if ($type_cetak == 1) {
						...
						foreach ($dt_wp as $row_wp) {
							...
							//$baris = $no_baris + 1;
							//$worksheet1->write_formula($no_baris, 10, "=J$bar	is-G$baris", $fsisa);
						...
						if ($lap_realisasi == 1) {
							...
							foreach ($dt_rekap as $row) {
								...
								$worksheet1->write_string($no_baris, 3, strtoupper($row['wp_wr_almt']), $fdata);
								$worksheet1->write_string($no_baris, 4,  date('F', strtotime($row['spt_periode_jual1'])), $fdata_center);
								$worksheet1->write_number($no_baris, 5, ceil((strtotime($row['spt_periode_jual1']) + $ut_to_ed_diff) / $seconds_in_a_day), $fdate);
								$worksheet1->write_string($no_baris, 6, $row['spt_nomor'], $fdata_center);
								$worksheet1->write_number($no_baris, 7, $row['spt_pajak'], $fcurrency);
								$worksheet1->write_string($no_baris, 8, $row['skbh_no'], $fdata_center);
	...
	function cetak_daftar_stpd($type_cetak = 0, $lap_realisasi = 1) {
		...
		// HTTP headers
		HeaderingExcel('daftar-rekapitulasi-stpd.xls');
		...
		if ($_GET['jenis_pajak'] != "4") {			
			...
			if (count($dt_kecamatan) > 0) {
				foreach ($dt_kecamatan as $camat) {
					...
					$worksheet1->write_blank($no_baris, 0, $fkecamatan);
					$worksheet1->write_string($no_baris, 1, $camat['wp_wr_camat'], $fkecamatan);
					$worksheet1->write_blank($no_baris, 2, $fkecamatan);
					if ($_GET['status_spt'] == "8") {
						for ($i = 3; $i <= 12; $i++)
							$worksheet1->write_blank($no_baris, $i, $fkecamatan);
					...
					if ($type_cetak == 1) {
						...
						if ($lap_realisasi == 1) {
							...
							foreach ($dt_rekap as $row) {
								...
								$worksheet1->write_string($no_baris, 2, $row['npwprd'], $fdata_center);
								$worksheet1->write_string($no_baris, 3, strtoupper($row['wp_wr_almt']), $fdata);
								$worksheet1->write_string($no_baris, 4,  date('F', strtotime($row['stpd_periode_jual1'])), $fdata_center);
								$worksheet1->write_number($no_baris, 5, ceil((strtotime($row['stpd_periode_jual1']) + $ut_to_ed_diff) / $seconds_in_a_day), $fdate);
								$worksheet1->write_string($no_baris, 6, $row['stpd_nomor'], $fdata_center);
								$worksheet1->write_number($no_baris, 7, $row['stpd_pajak'], $fcurrency);
								$worksheet1->write_string($no_baris, 8, $row['stpd_nomor_spt'], $fdata_center);
								
								if ($row['stpd_tgl_setoran'] != NULL)
									$worksheet1->write_number($no_baris, 9, ceil((strtotime($row['stpd_tgl_setoran']) + $ut_to_ed_diff) / $seconds_in_a_day), $fdate);
								else 
									$worksheet1->write_blank($no_baris, 9, $fdate);
									
								if ($row['stpd_tgl_setoran'] != NULL)
									$worksheet1->write_number($no_baris, 10, $row['stpd_sanksi'], $fcurrency);
								else
									$worksheet1->write_blank($no_baris, 10, $fdata);
			...
			foreach ($dt_rekap as $row) {
				...
				$worksheet1->write_string($no_baris, 2, $row['npwprd'], $fdata_center);
				$worksheet1->write_string($no_baris, 3, strtoupper($row['wp_wr_almt']), $fdata);
				$worksheet1->write_number($no_baris, 4, ceil((strtotime($row['stpd_periode_jual1']) + $ut_to_ed_diff) / $seconds_in_a_day), $fdate);
				$worksheet1->write_number($no_baris, 5, ceil((strtotime($row['stpd_periode_jual1']) + $ut_to_ed_diff) / $seconds_in_a_day), $fdate);
				$worksheet1->write_string($no_baris, 6, $row['stpd_nomor'], $fdata_center);
				$worksheet1->write_number($no_baris, 7, $row['stpd_pajak'], $fcurrency);
				$worksheet1->write_string($no_baris, 8, $row['stpd_nomor_spt'], $fdata_center);
