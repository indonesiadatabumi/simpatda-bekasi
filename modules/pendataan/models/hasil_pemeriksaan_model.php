...
class Hasil_pemeriksaan_model extends CI_Model {
	...
	function insert() {
		...
		$lhp_nomor = $this->input->post('lhp_no_periksa');
		$kd_billing = "24".$lhp_nomor."".date("dmy")."".rand(1111,9999);
		//save header
		$arr_lhp = array(
						'lhp_id' => $lhp_id,
	...
	function get_data_lhp($lhp_id) {
		
		$sql = "SELECT * FROM laporan_hasil_pemeriksaan a 
				LEFT JOIN laporan_hasil_pemeriksaan_detail b ON a.lhp_id=b.lhp_dt_id_header
				WHERE a.lhp_id = $lhp_id";
		// echo $sql;die;
		$query = $this->db->query($sql);
	...
	function history_del_lhp($data_lhp) {
		$data = array(
				'spt_id' => $data_lhp->lhp_id,
				'spt_periode' => $data_lhp->lhp_periode,
				'spt_nomor' => $data_lhp->lhp_no_periksa,
				'spt_kode_rek' => $data_lhp->lhp_kode_rek,
				'spt_periode_jual1' => $data_lhp->lhp_dt_periode1,
				'spt_periode_jual2' => $data_lhp->lhp_dt_periode2,
				'spt_nilai' => $data_lhp->lhp_pajak,
				'spt_pajak' => $data_lhp->lhp_pajak,
				'spt_operator' => $data_lhp->spt_operator,
				'spt_jenis_pajakretribusi' => $data_lhp->lhp_jenis_pajakretribusi,
				'spt_idwpwr' => $data_lhp->lhp_idwpwr,
				'spt_tgl_proses' => $data_lhp->lhp_tgl,
				'spt_tgl_entry' => $data_lhp->lhp_tgl,
				'spt_tarif_persen' => $data_lhp->lhp_dt_tarif_persen,
				'spt_kode_billing' => $data_lhp->lhp_kode_billing,
				'status_bayar' => $data_lhp->lhp_status_bayar
			);
