...
class Pajak_restoran extends Master_Controller {
	...
	function update() {
		...
		if (!empty($wp_id)) {
			...
			if (!$check_masa_pajak) {
				...
				if ($return) {
					$cek_data_detil - $this->spt_model->cek_data_detil_resto($spt_id);
					if ($cek_data_detil->spt_dt_id_spt == '0') {//jika data pada tbel spt_detail kosong
						$this->spt_model->insert_spt_detail($spt_id);
					}else{
	...
	function delete() {
		...
		for ($i = 0; $i < count($arr_id) - 1; $i++) {
			// get data spt
			$data_spt = $this->spt_model->get_data_spt($arr_id[$i]);
			...
			$this->spt_model->history_del_spt($data_spt);
		}
