...
$tahun_anggaran = $arr_tgl_proses[2];
 ...
 // HTTP headers
HeaderingExcel('daftar_wp_belum_lapor.xls');
...
$worksheet1->write_string(0,0, "DAFTAR WP BELUM LAPOR " . strtoupper($jenis_pajak->ref_jenparet_ket), $ftitle); 
...
if ($_GET['tahun'] == $_GET['tahun2']){
	...
	$worksheet1->write_string(1,0, "TAHUN : " . $_GET['tahun']. " - ". $_GET['tahun2'], $ftitle); 
	$worksheet1->merge_cells(1, 0, 1, 9);
}

if ($_GET['masa_pajak'] == $_GET['masa_pajak2']){
	$worksheet1->write_string(2,0, "MASA PAJAK : ".strtoupper(getNamaBulan($_GET['masa_pajak'])), $ftitle); 
	$worksheet1->merge_cells(2, 0, 2, 9);
}else{
	$worksheet1->write_string(2,0, "MASA PAJAK : ".strtoupper(getNamaBulan($_GET['masa_pajak']). " - ".strtoupper(getNamaBulan($_GET['masa_pajak2']))), $ftitle); 
	$worksheet1->merge_cells(2, 0, 2, 9);
}
	...
	foreach($rows as $key => $val) {
		if ($_GET['spt_jenis_pajakretribusi'] == "3" || $_GET['spt_jenis_pajakretribusi'] == "7") {
			...
			if ($row_span > 0) {
				...
				$worksheet1->write($no_baris,6, getNamaBulan(date("m",strtotime($val['spt_periode_jual1'])))." ".date("Y",strtotime($val['spt_periode_jual1'])),$fdata);
			...
			$worksheet1->write($no_baris,2, $val['spt_nomor'],$fdata);
			$worksheet1->write($no_baris,3, $val['wp_wr_nama'],$fdata);
			$worksheet1->write($no_baris,4, preg_replace("\r|\n|","",$val['wp_wr_almt']),$fdata);
			$worksheet1->write($no_baris,5, $val['npwprd'],$fdata);
			$worksheet1->write($no_baris,6, getNamaBulan(date("m",strtotime($val['spt_periode_jual1'])))." ".date("Y",strtotime($val['spt_periode_jual1'])),$fdata);
