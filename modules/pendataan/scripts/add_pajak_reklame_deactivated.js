...
var calcPajak = function() {
	...
	if (no_rekening == "4.1.1.04.06") {
		durasi = parseInt($("#txt_durasi").val()*1);
		nilai = durasi * jumlah * ($('#txt_nilai_tarif').val().toString().replace(/\$|\,00/g,'').replace(/\$|\./g,'') * 1);
...
var completePage = function() {
	...
	$("#spt_dt_korek").change(function(){
		...
		if (no_rekening == "4.1.1.04.01" || no_rekening == "4.1.1.04.02") {
			...
			$("#spt_dt_persen_tarif").val(arr_spt_korek[1]);
			insertKelasJalan();
		} else {
			$("#row_persen_tarif").hide();
			$("#spt_dt_persen_tarif").val(arr_spt_korek[1]);
			$("#txt_nilai_tarif").val(formatCurrency(arr_spt_korek[2]));
		}
		...
		rowField = 
			'<table>'+
				...
				'<tr>'+
					...
					'<td><input type="text" name="txt_jumlah" id="txt_jumlah" class="inputbox" size="5" onchange="calcPajak();" onblur="calcPajak();"> unit</td>' +
				'</tr>' +
				
                		'<tr>'+
					'<td class="key">Lama Pemasangan</td>' +
					'<td><input type="text" name="txt_durasi" id="txt_durasi" class="inputbox" size="6" onchange="calcPajak();" onblur="calcPajak();"> bulan</td>' +
				'</tr>' +				
				'<tr>'+
		...
		rowField = 
			'<table>'+
				'<tr>'+
					...
					'<td><input type="text" name="txt_luas" id="txt_luas" class="inputbox" size="8" onchange="calcPajak();"> Cm<sup>2</td>' +
				'</tr>' +
