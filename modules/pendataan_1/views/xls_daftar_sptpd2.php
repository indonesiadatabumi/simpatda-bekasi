...
$worksheet1->write_string(1,0, "TANGGAL : " . format_tgl($_GET['tgl_entry1'],false,true), $ftitle); 
$worksheet1->merge_cells(1, 0, 1, 9);
$worksheet1->write_string(2,0, "S/D :" . format_tgl($_GET['tgl_entry2'],false,true), $ftitle); 
...
if (count($rows) > 0) {		
	foreach($rows as $key => $val) {
		if ($_GET['spt_jenis_pajakretribusi'] == "3" || $_GET['spt_jenis_pajakretribusi'] == "7") {
			...
			$tahun_periode = substr($val['spt_periode_jual1'],0,4);
			$bulan_periode = substr($val['spt_periode_jual1'],5,2);
			...
			if ($row_span > 0) {
				...
				$worksheet1->write($no_baris,6, getNamaBulan($bulan_periode)." ".$tahun_periode,$fdata);
			...
			$arr_tgl_periode = explode('-', $val['spt_periode_jual1']);
			$tahun_periode = substr($val['spt_periode_jual1'],0,4);
			$bulan_periode = substr($val['spt_periode_jual1'],5,2);
		...
		//	$worksheet1->write($no_baris,6, getNamaBulan($arr_periode_jual[1])." ".$arr_periode_jual[0],$fdata);
			$worksheet1->write($no_baris,6, getNamaBulan($bulan_periode)." ".$tahun_periode,$fdata);
			//getNamaBulan($_GET['masa_pajak'])." ".$_GET["tahun"],$fdata);
