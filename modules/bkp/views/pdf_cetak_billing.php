		...
		if (@$_GET['jenis_setoran'] == "1" || @$_GET['jenis_setoran'] == null || @empty($_GET['jenis_setoran'])) {
			if (count($ar_rekam_setor) > 0) {
				...
				if ($ar_rekam_setor[0]['tgl_jatuh_tempo'] != null  && $_REQUEST['spt_jenis_pajakretribusi'] != "'") {
					...
					if ($diff_month > 0)
						$denda = ceil(0.01 * $ar_rekam_setor[0]['spt_pajak'] * $diff_month);
				}
			}	
		}
...
<body onLoad='window.print()'>
	...
	<table border=0 style="width: 500px; font-family:">
	<tr>
	...
	<td valign="top" style="font-weight:bold; text-align:center;">
			<span style="font-size:20px; ">PEMERINTAH KOTA BEKASI</span> <br />
			<span style="font-size:16px; ">BADAN PENDAPATAN DAERAH</span> <br />
		...
		<div style="width:500px; border-bottom:1px solid; "></div>
		
		
		<table border=0 style="width: 500px; font-size:12px;">
			<tr>
				<td style="width:100px; ">No. KOHIR</td><td colspan="2" width="300">: <?php echo  $ar_rekam_setor[0]['spt_nomor'];?></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<tr><td style="width:200px; text-align:center; font-size:18px; font-weight:bold; "> <?php echo $ar_rekam_setor[0][spt_kode_billing];?></td></tr>
				</table>
				</td>
			</tr>
			<tr>
				<td>Jenis Pajak</td>
				<td>: <?php echo  $v['korek_nama_detail'];?></td>
			</tr>
			
			<tr>
				<td>Naskah </td>
				<td>: <?php echo  $v['sptrek_judul'];?></td>
			</tr>
	...
	<table>
	<tr>
		<td style="font-size:9px; ">Tanggal Cetak :  <?php echo $tgl_cetakan;?></td>
	</tr>
	</table>	 
	<div style="font-size:9px; width:500px; font-weight:bold; ">** Keterlambatan Pembayaran melewati tanggal jatuh tempo akan dikenakan sanksi administrasi berupa bunga
	sebesar 2% (dua persen) setiap bulannya </div>
	<div>&nbsp;</div>
	<div style="font-weight:bold; text-align:center; width:400px; font-size:11px; ">"PAJAK ANDA MEMBANGUN KOTA BEKASI" <br />-- TERIMA KASIH --</div>
</div>
<br>
<br>
<br>
