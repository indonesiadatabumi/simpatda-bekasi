...
class Rekam_pajak extends Master_Controller {
	...
	function cetak_sspd_pdf() {
		...
		if (@$_GET['jenis_setoran'] == "1" || @$_GET['jenis_setoran'] == null || @empty($_GET['jenis_setoran'])) {
			if (count($arr_sspd) > 0) {
				...
				if ($arr_sspd[0]['tgl_jatuh_tempo'] != null  ) {
	...
	function proses_setoran() {
		...
		// if ($this->input->get_post('spt_jenis_pajakretribusi') == '8' || $this->input->get_post('spt_jenis_pajakretribusi') == '4') {
		// 	$jatuh_tempo = date('Y-m-d', strtotime('+1 month', strtotime($arr_sspd[0]['tgl_jatuh_tempo'])));
		// }else {
		// 	$jatuh_tempo = date('Y-m-10', strtotime('+1 month', strtotime($arr_sspd[0]['spt_periode_jual1'])));
		// }
		if ($this->input->get_post('spt_jenis_pajakretribusi') == '8' || $this->input->get_post('spt_jenis_pajakretribusi') == '4') {
			$jatuh_tempo = $arr_sspd[0]['tgl_jatuh_tempo'];
		}else {
			$jatuh_tempo = date('Y-m-10', strtotime('+1 month', strtotime($arr_sspd[0]['spt_periode_jual1'])));
		}
		
		if (count($arr_sspd) > 0) {
			if ($jatuh_tempo != null && $_POST['tanggal_setor'] != "") {
				// $diff_month = get_diff_months($jatuh_tempo, format_tgl($this->input->post('tanggal_setor')), $arr_sspd[0]['ketspt_id']);
		
				// if ($diff_month > 25)
				// 	$denda = ceil(0.01 * $arr_sspd[0]['spt_pajak'] * 24);
					
				// 	else if 
				// 	($diff_month < 25)
				// 	$denda = ceil(0.01 * $arr_sspd[0]['spt_pajak'] * $diff_month);
				$denda = denda($jatuh_tempo, format_tgl($this->input->post('tanggal_setor')), $arr_sspd[0]['spt_pajak']);
		
			}
		
		}

		if ($this->input->get_post('setorpajret_jenis_ketetapan') == '3' || $this->input->get_post('setorpajret_jenis_ketetapan') == '11') {
			$denda = 0;
		}
