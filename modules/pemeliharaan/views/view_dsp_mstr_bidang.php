...
<a href="#" onclick="javascript:if(document.adminForm.boxchecked.value==0){alert('Silahkan memilih dari daftar yang akan diedit');}else{ hideMainMenu(); submitbutton('edit')}" class="toolbar">
...
<a href="#" onclick="javascript:if(document.adminForm.boxchecked.value==0){alert('Silahkan memilih dari daftar yang akan dihapus');}else{  konfirm('remove')}" class="toolbar">
...
<td class="button" id="toolbar-new">
<a href="#" onclick="openChildGB('Daftar Urusan','<?//= $myself.$XFA['dsp_ref_urusan'];?>','win2')" class="toolbar">
<span class="icon-32-ref" title="Golongan">
</span>
Daftar Urusan
</a>
</td>
...
Tabel Bidang
		...
		<div id="element-box">
			...
			<div class="m">
						<form action="<?//= $myself.$XFA['act_mstr_bidang']; ?>" method="post" name="adminForm">
...
<span id="saveBar"></span>
		...
		<input type="hidden" name="master" value="pejabat" />
...
<!--this is used for manipulation only (by bayu)-->
<div id="formSelect_urusan" style="display:none;"></div>
<div id="formSelect_fungsi" style="display:none;"></div>
