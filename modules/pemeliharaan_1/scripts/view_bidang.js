...
var createDataGrid = function (){
	$("#tbl_bidang").flexigrid({
		url: GLOBAL_BIDANG_VARS["get_list"],
		dataType: 'json',
		colModel : [
			{display: 'ID', name : 'bdg_id', width : 200, sortable : true, align: 'left', hide:true, process: cellClick},
			...
			{display: 'Kode', name : 'bdg_kode', width : 100, sortable : true, align: 'center', process: cellClick},
			{display: 'Nama', name : 'bdg_nama', width : 350, sortable : true, align: 'left', process: cellClick},
			{display: 'Fungsi', name : 'fung_nama', width : 300, sortable : true, align: 'left', process: cellClick}
		],
		searchitems : [
			{display: 'Kode', name : 'bdg_kode', isdefault: true},
			{display: 'Nama', name : 'bdg_nama'},
			{display: 'Urusan', name : 'ref_urus_nama'},
			{display: 'Fungsi', name : 'fung_nama'}
		],
		sortname: "bdg_id",
		sortorder: "asc",
		usepager: true,
...
var selectRow = function(index, id) {
 ...
 var createEventToolbar = function (){	 	
 	$("#btn_add").click(function (){
 		showDialog(GLOBAL_BIDANG_VARS["add"], "Bidang", 700, 300);
 	});
 	
 	$("#btn_edit").click(function (){ 		
 		if ($("input[name=id]").val().length > 0) {
 			load_content(GLOBAL_BIDANG_VARS["edit"], {
 	...
 	$("#btn_jabatan").click(function() {
 		showDialog(GLOBAL_BIDANG_VARS["form_jabatan"], 'Daftar Jabatan', 950, 550);
 	});
 	
 	$("#btn_golongan").click(function() {
 		showDialog(GLOBAL_BIDANG_VARS["form_golongan"], 'Daftar Golongan', 950, 550);
 	});
 };
