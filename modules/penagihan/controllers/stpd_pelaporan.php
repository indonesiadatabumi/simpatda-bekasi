 ...
 * @version 20121016
 */
class Stpd_pelaporan extends Master_Controller {
	/**
	 * construct
	 */
	function __construct() {
		parent::__construct();
		$this->load->model('stpd_pelaporan_model');
	 ...
	 */
	function index() {
		$this->load->view('view_stpd_pelaporan');
	}
	 ...
	 */
	function get_list() {
		$this->stpd_pelaporan_model->get_list();
	}
	...
	function get_spt() {
		...
		$data['flexigrid_url'] = base_url()."penagihan/stpd_pelaporan/get_calon_stpd?sqtype=&squery=".$get;
		$this->load->view('popup_stpd_pelaporan', $data);
	}
	...
	function get_calon_stpd() {
		$this->stpd_pelaporan_model->get_calon_stpd();
	}
	...
	function get_detail_setoran() {
		$query = $this->stpd_pelaporan_model->get_detail_setoran($_POST['setorpajret_id']);
		...
		if ($query != null || $query->num_rows() > 0) {
			...
			if ($row->wp_wr_gol == '2'){
				$persen_denda = '100.000';
				$pajak = 100000 * 1;
			}else {
				$persen_denda == '50.000';
				$pajak = 50000 * 1;
			}
			$jatuh_tempo = date("Y-m-15", strtotime($row->setorpajret_jatuh_tempo));
			
			if ($row->tgl_lapor == null) {
				$tgl_lapor = date('Y-m-20');
			}else {
			...
			$result = array(
					...
					'setorpajret_jatuh_tempo' => format_tgl($jatuh_tempo),
					'setorpajret_tgl_bayar' => format_tgl($tgl_lapor),
	...
	function add() {
		...
		$this->load->view('add_stpd_pelaporan', $data);
	}
	
	function insert() {
		$result = $this->stpd_pelaporan_model->insert();
		echo json_encode($result);
	}
	...
	function edit() {		
		...
		$this->load->view('edit_stpd_pelaporan', $data);
	}
	 ...
	 */
	function update() {
		$result = $this->stpd_pelaporan_model->update();
		echo json_encode($result);
	}
	...
	function delete() {
		...
		for ($i = 0; $i < count($arr_id) - 1; $i++) {
			// get data spt
			$data_stpd = $this->stpd_pelaporan_model->get_data_stpd($arr_id[$i]);

			if($this->stpd_pelaporan_model->delete($arr_id[$i]) == true) {				
				$counter++;
				//insert history log
			...
			$this->stpd_pelaporan_model->history_del_spt($data_stpd);
		}
	...
	function get_list_data_stpd() {
		$this->stpd_pelaporan_model->get_list_data_stpd();
	}
	...
	function pdf_stpd() {
		...
		$data['model'] = $this->stpd_pelaporan_model;
	 ...
	 * cetak daftar stpd
	 */
	function cetak_daftar_stpd_pelaporan() {
		...
		$this->load->view('form_cetak_daftar_stpd_pelaporan', $data);
	}
	...
	function pdf_cetak_daftar() {
		...
		$data['query'] = $this->stpd_pelaporan_model->get_daftar_stpd();
		
		//insert history log
		$this->common_model->history_log("penagihan", "P", "Print Daftar STPD : ".$_GET['jenis_pajak']." | ".$_GET['bulan']." | ".$_GET['tahun']." | ".$_GET['camat_id']);
		
		$this->load->view('pdf_daftar_stpd_pelaporan', $data);
	}

	/**
	 * cetak daftar stpd2
	 */
	function pdf_cetak_daftar2() {
		$mengetahui = $this->common_model->get_query('*', 'v_pejabat_daerah', "pejda_id='".$this->input->get('mengetahui')."'");
		$data['mengetahui'] = $mengetahui->row();
		...
		$data['jenis_pajak'] = $this->common_model->get_record_value('ref_jenparet_ket', 'ref_jenis_pajak_retribusi', "ref_jenparet_id='".$this->input->get('jenis_pajak')."'");
		$data['query'] = $this->stpd_pelaporan_model->get_daftar_stpd2();
		
		//insert history log
		$this->common_model->history_log("penagihan", "P", "Print Daftar STPD : ".$_GET['jenis_pajak']." | ".$_GET['tgl_penetapan1']." | ".$_GET['tgl_penetapan2']." | ".$_GET['camat_id']);
		
		$this->load->view('pdf_daftar_stpd_pelaporan2', $data);
	}
