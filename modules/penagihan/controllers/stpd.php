...
class Stpd extends Master_Controller {
	...
	function get_detail_setoran() {
		...
		if ($query != null || $query->num_rows() > 0) {
			...
			if ($row->setorpajret_periode_jual1 < '2024-01-01'){
				$persen_denda = '2';
				$pajak = round($row->setorpajret_jlh_bayar * $bulan_pengenaan * (int)$persen_denda / 100);
			}else {
				$persen_denda = '1';
				$pajak = round(($row->setorpajret_jlh_bayar * $bulan_pengenaan * (int)$persen_denda) / 100);
			}
	...
	function delete() {
		...
		for ($i = 0; $i < count($arr_id) - 1; $i++) {
			// get data spt
			$data_stpd = $this->stpd_model->get_data_stpd($arr_id[$i]);
			...
			$this->stpd_model->history_del_spt($data_stpd);
		}
	...
	function pdf_cetak_daftar2() {
		...
		$data['jenis_pajak'] = $this->common_model->get_record_value('ref_jenparet_ket', 'ref_jenis_pajak_retribusi', "ref_jenparet_id='".$this->input->get('jenis_pajak')."'");
		$data['query'] = $this->stpd_model->get_daftar_stpd2();
		...
		$this->load->view('pdf_daftar_stpd2', $data);
	}
