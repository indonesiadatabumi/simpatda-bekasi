...
class Stpd_model extends CI_Model {
	...
	function get_list() {
		...
		$where = " WHERE stpd_id IS NOT NULL and sanksi_lapor='0'";
	...
	function insert() {
		...
		$sql = "SELECT * FROM stpd WHERE stpd_setoran_id=".$_POST['setorpajret_id']." AND sanksi_lapor='0'";
		$query = $this->db->query($sql);
		...
		$kd_billing = $_POST['stpd_nomor']."".format_angka($this->config->item('length_no_spt'), $stpd_nomor)."".date("dmy")."".rand(11111,99999);
		$arr_insert = array(
						'stpd_id' => $stpd_id,
	...
	function get_list_cetak_stpd($periode, $jenis_pajak, $stpd_nomor1, $stpd_nomor2) {
		$sql = "SELECT a.*, b.sanksi_lapor FROM v_stpd a LEFT JOIN stpd b ON a.stpd_id=b.stpd_id WHERE a.stpd_periode=$periode AND a.stpd_jenis_pajak=$jenis_pajak AND a.stpd_nomor BETWEEN $stpd_nomor1 AND $stpd_nomor2";
		// echo $sql;
	...
	function get_daftar_stpd() {
		$where = "stpd_id IS NOT NULL and sanksi_lapor='0'";
		...
		$sql = "SELECT * FROM v_stpd_pembayaran WHERE $where ORDER BY stpd_nomor ASC";
		$query = $this->adodb->GetAll($sql);
