 ...
 */
class Stpd_pelaporan_model extends CI_Model {
	/**
	 * get list stpd
	 */
	function get_list() {
		$tables = " v_stpd_pelaporan ";
		...
		$where = " WHERE stpd_id IS NOT NULL AND sanksi_lapor='1'";
			...
			while ($row = $result->FetchNextObject(false)) {
				$list[] = array (
								...
								"cell"	=> array (
									...
									$row->koderek_titik,									
									$row->korek_nama,
									format_tgl($row->stpd_tgl_setoran),
	...
	function get_calon_stpd() {
		$tables = " v_calon_stprd a
					LEFT JOIN spt b ON a.setorpajret_id_spt=b.spt_id";
	
		$sortname = $_POST['sortname'];
		$sortorder = $_POST['sortorder'];
		if (!$sortname) $sortname = ' a.spt_id ';
		if (!$sortorder) $sortorder = 'desc';
		$sort = "ORDER BY $sortname $sortorder";
		...
		$where = " WHERE a.setorpajret_jenis_pajakretribusi='".$_GET['jenis_pajak']."' AND EXTRACT(MONTH FROM a.setorpajret_periode_jual1) = '".$_GET['bulan_realisasi']."'
						AND EXTRACT(YEAR FROM a.setorpajret_periode_jual1) = '".$_GET['tahun_realisasi']."' AND (EXTRACT(DAY FROM b.tgl_lapor) > '16' OR b.tgl_lapor IS NULL)";
		...
		if ($result) {
			while ($row = $result->FetchNextObject(false)) {
				...
				$bulan_pengenaan1 = bulan_pengenaan($row->setorpajret_jatuh_tempo, $row->setorpajret_tgl_bayar, $row->setorpajret_jenis_ketetapan);
				if ($bulan_pengenaan1 > 15){
					$bulan_pengenaan = 15;
				}else if($row->setorpajret_periode_jual1 >= '2024-02-01' && $row->setorpajret_periode_jual1 <= '2024-02-29' && $row->setorpajret_tgl_bayar <= '2024-03-13'){
					$bulan_pengenaan = 0;
				}else {
	...
	function get_detail_setoran($setoran_id) {
		...
		$sql = "SELECT a.*, b.tgl_lapor FROM v_calon_stprd a
		LEFT JOIN spt b ON a.setorpajret_id_spt=b.spt_id WHERE setorpajret_id=".$setoran_id;
	...
	function insert() {
		...
		$sql = "SELECT * FROM stpd WHERE stpd_setoran_id=".$_POST['setorpajret_id']."AND sanksi_lapor='1'";
		$query = $this->db->query($sql);
		...
		$arr_insert = array(
						...
						'stpd_jatuh_tempo' => format_tgl($_POST['setorpajret_jatuh_tempo']),
	...
	function get_daftar_stpd() {
		$where = "stpd_id IS NOT NULL and sanksi_lapor = '1'";
		...
		$sql = "SELECT * FROM v_stpd_pelaporan WHERE $where ORDER BY stpd_nomor ASC";
		$query = $this->adodb->GetAll($sql);
	 ...
	 * get daftar stpd2
	 */
	function get_daftar_stpd2() {
		$tgl_penetapan1 = format_tgl($_GET['tgl_penetapan1']);
		$tgl_penetapan2 = format_tgl($_GET['tgl_penetapan2']);
		$where = "stpd_id IS NOT NULL and sanksi_lapor = '1'";
		
		if ($tgl_penetapan1 != "" && $tgl_penetapan2 != "") {
			$where .= " AND stpd_tgl_proses >= '$tgl_penetapan1' AND stpd_tgl_proses <= '$tgl_penetapan2'";
		}
	...
	function get_data_stpd($stpd_id) {
		
		$sql = "SELECT * FROM stpd WHERE stpd_id = $stpd_id";
		// echo $sql;die;
		$query = $this->db->query($sql);
	...
	// insert tabel history_del_spt
	function history_del_spt($data_stpd) {
		$data = array(
				'spt_id' => $data_stpd->stpd_id,
				'spt_periode' => $data_stpd->stpd_periode,
				'spt_nomor' => $data_stpd->stpd_nomor,
				'spt_kode_rek' => $data_stpd->stpd_korek_id,
				'spt_periode_jual1' => $data_stpd->stpd_periode_jual1,
				'spt_periode_jual2' => $data_stpd->stpd_periode_jual2,
				'spt_pajak' => $data_stpd->stpd_pajak,
				'spt_jenis_pajakretribusi' => $data_stpd->stpd_jenis_pajak,
				'spt_idwpwr' => $data_stpd->stpd_wp_id,
				'spt_tgl_proses' => $data_stpd->stpd_tgl_proses,
				'spt_tgl_entry' => $data_stpd->stpd_tgl_proses,
				'spt_kode_billing' => $data_stpd->stpd_kode_billing,
				'status_bayar' => $data_stpd->status_bayar
			);
			
		$this->db->insert('history_del_spt', $data);
