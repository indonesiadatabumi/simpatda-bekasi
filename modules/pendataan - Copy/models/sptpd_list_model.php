...
class Sptpd_list_model extends CI_Model {
	...
	function get_list_spt_by_tanggal_entry() {
		...
		if($spt_jenis_pajak_restoran <> '' AND $spt_jenis_pajak == 2){
			$sql_cari .= " AND c.sptresto_jenis='".$spt_jenis_pajak_restoran."'";
		}
	...
	function get_daftar_sptpd_restoran() {
		...
		if ($this->input->get('daftar_spt') == '2' || $this->input->get('daftar_spt') == '0') {
			...
			$sql_cari = "SELECT *, COALESCE(sptresto_jenis, 1) as sptrestoran_jenis
			...
			if ($jenis_restoran != "0")
				$sql_cari .= " AND COALESCE(sptresto_jenis, 1)='".$jenis_restoran."'";
