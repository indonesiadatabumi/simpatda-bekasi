 ...
 * @version 20121022
 */
class Pajak_restoran_model extends CI_Model {
/**
	 * get list data
	 */
	function get_list() {
		...
		$where = " WHERE spt_jenis_pajakretribusi='".$this->config->item('jenis_pajak_restoran')."'";
			...
			while ($row = $result->FetchNextObject(false)) {
				$list[] = array (
								...
								"cell"	=> array (
									...
									//format_tgl($row->spt_tgl_terima),
									//format_tgl($row->spt_tgl_bts_kembali)
								)
							);
				$counter++;
			}	
	...
	//insert into spt_restoran
	function insert_spt_restoran($spt_id) {
		$arr_insert = array(
						'sptresto_id_spt' => $spt_id,
						'sptresto_jenis' => $this->input->post('jenis_pajak_restoran')
					);
		$this->db->insert('spt_restoran', $arr_insert);
	}
	
	/**
	 * update spt_restoran table
	 * @param unknown_type $spt_id
	 */
	function update_spt_restoran($spt_id) {
		$query = $this->db->get_where('spt_restoran', "sptresto_id_spt = '$spt_id'");
		
		if ($query->num_rows() > 0) {
			//update
			$row = $query->row(0);
			$sptresto_id = $row->sptresto_id;
			
			$arr_update = array(
						'sptresto_jenis' => $this->input->post('jenis_pajak_restoran')
					);
			$this->db->where('sptresto_id', $sptresto_id);
			$this->db->update('spt_restoran', $arr_update);
		} else {
			$this->insert_spt_restoran($spt_id);
		}
	}
	 ...
	 * @param unknown_type $spt_id
	 */
	function get_spt_restoran($spt_id) {
		$result = 1;
		$query = $this->db->get_where('spt_restoran', "sptresto_id_spt = '$spt_id'");
		
		if ($query->num_rows() > 0) {
			$row = $query->row(0);
			$result = $row->sptresto_jenis;
		}
	 ...
	 * check_masa_pajak
	 */
	function check_masa_pajak($wpwr_id, $spt_periode_jual1, $spt_periode_jual2, $jenis_pungutan = 1, $spt_id = "", $jenis_restoran) {		
		$where .= " spt_periode_jual1 BETWEEN '".format_tgl($spt_periode_jual1)."' AND '".format_tgl($spt_periode_jual2)."' 
					...
					AND spt_jenis_pemungutan='$jenis_pungutan' AND spt_idwpwr='".$wpwr_id."'";
		...
		if ($jenis_restoran == 1) 
			$where .= " AND sptresto_jenis = 1";
		
		$sql = "SELECT *
					FROM spt LEFT JOIN spt_restoran ON spt.spt_id = spt_restoran.sptresto_id_spt 
					WHERE $where";
